...
from dr_config import randomize_environment_params
...
def save_dr_csv(dr_dict, episode_id):
    ...
    file_exists = os.path.exists("domain_ranges.csv")
    with open("domain_ranges.csv", "a", newline='', encoding='utf-8') as f:
...
def load_eval_metrics():
    try:
        with open("eval_metrics.json", "r", encoding="utf-8") as f:
            return json.load(f)
    except:
        return {}
...
def main():
    import SAC_training  # 动态引入，避免提前初始化模型
    SAC_training.init_training()
    ...
    for episode in range(1, TOTAL_EPISODES + 1):
        ...
        with open(f"round_summary_{episode}.json", "w", encoding="utf-8") as f:
    ...
    with open("reward_history.json", "w", encoding="utf-8") as f:
    ...
    torch.save(SAC_training.model_action.state_dict(), 'model_action.pth')
    torch.save(SAC_training.model_value1.state_dict(), 'model_value1.pth')
    torch.save(SAC_training.model_value2.state_dict(), 'model_value2.pth')
