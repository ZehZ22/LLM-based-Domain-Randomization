...
class USVStateR:
    def __init__(self, waypoints, current_index, x, ui, model_func=marinerwind, wind_speed=2.0, wind_direction=330.0,
                 ...
                 wave_height=2.0, wave_period=3.0, beta=75 * np.pi / 180, T4=3.0, GMT=1.0, Cb=0.65, U=7.7175, L=160.93,
        ...
        if randomize_params:
            ...
            self.wind_direction = random.choice([0.0, 45.0, 90.0, 135.0])  # 风向范围 [0,360] 度
            self.current_speed = self.randomize_value(1.0, 1.5, no_seed_random)  # 洋流速度范围 [2,3] knot
            self.current_direction = random.choice([0.0, 45.0, 90.0, 135.0])   # 洋流方向范围 [0,360] 度
            self.wave_height = self.randomize_value(3.0, 4.0, no_seed_random)  # 波高范围 [0.5,3] m
            self.beta = np.radians(random.choice([0.0, 45.0, 90.0, 135.0]))  # 转换为弧度
    ...
    def step(self, ui):
        ...
        if np.isnan(xdot).any() or np.isinf(xdot).any() or (np.abs(xdot) > 1e6).any():
            ...
            xdot = np.nan_to_num(xdot, nan=0.0, posinf=1e6, neginf=-1e6)
            xdot = np.clip(xdot, -1e6, 1e6)

        self.x += xdot * dt  # 应用合法后的 xdot 更新状态
        ...
        # ✅ 状态非法保护（新增）
        if np.isnan(self.x).any() or np.isinf(self.x).any() or (np.abs(self.x) > 1e6).any():
            print(f"❌ 状态 self.x 爆炸，提前终止 episode：{self.x}")
            dummy_state = np.array([0.0, 0.0, 0.0], dtype=np.float32)
            return dummy_state, -1e6, True, {}  # 惩罚性奖励，提前 done
        ...
        heading_error_scalar = np.clip(self.heading_error, -1e3, 1e3)
        cross_track_error_scalar = np.clip(self.cross_track_error, -1e3, 1e3)
        rudder_angle = np.clip(self.x[6], -1e2, 1e2)
        ...
        next_state = np.array([heading_error_scalar, cross_track_error_scalar, rudder_angle], dtype=np.float32)
